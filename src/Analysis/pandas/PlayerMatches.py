...
def show_player_matches(players_df, matches, sport: Sport, competition: Competition = None):
    if sport.nom == "tennis":
        ...
        if len(players_df) == 1:
            ...
            index = int(input("Sélectionne un joueur (numéro) : "))
        pseudo = players_df.loc[index]["pseudo"]
        ...
        player_matches = matches_df[matches_df[player_cols].isin([pseudo]).any(axis=1)]
        if len(player_matches) == 0:
            print(f"Aucun match trouvé.")
        else:
            for _, match in player_matches.iterrows():
        ...
        if len(players_df) == 1:
            ...
            index = int(input("Sélectionne un joueur (numéro) : "))
        pseudo = players_df.loc[index]["pseudo"]
        team = players_df.loc[index]["team"]
        matches_df = pd.read_csv("./data/counter_strike_2/match.csv")
        player_matches = matches_df[(matches_df["team_1"] == team) | (matches_df["team_2"] == team)]
        if len(player_matches) == 0:
